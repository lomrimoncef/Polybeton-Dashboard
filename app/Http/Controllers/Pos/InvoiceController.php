        ...
        if ($invoice->save()) {
           ...
           for ($i=0; $i < $count_category ; $i++) {
              ...
              $invoice_details->selling_qty = $request->selling_qty[$i];
               $invoice_details->pu_vente = $request->unit_price[$i];
            ...
            $payment->tva = $request->tva[0]*$request->estimated_amount2;


            $payment->total_amount_ht=$request->estimated_amount2;

            if( $request->Timbre)
            { $payment->total_amount = $request->estimated_amount1+$request->estimated_amount1*0.01;
                $payment->timbre=$request->estimated_amount1*0.01;
            }

else {$payment->total_amount = $request->estimated_amount1;};
            ...
            if ($request->paid_status == 'full_paid') {
                ...
                $payment_details->current_paid_amount = $payment->total_amount;
            } elseif ($request->paid_status == 'full_due') {
                $payment->paid_amount = '0';
                $payment->due_amount = $payment->total_amount;
                $payment_details->current_paid_amount = '0';
            }
            elseif ($request->paid_status == 'Ristourne') {
                $payment->paid_amount = '0';
                $payment->due_amount = '0';
                $payment_details->current_paid_amount = '0';
    ...
    return redirect()->back()->with($notification); 
        ...
        DB::transaction(function() use($request,$invoice,$id){
            foreach($request->selling_qty as $key => $val){
             ...
             $product->quantity_vendu =   $product->quantity_vendu +$request->selling_qty[$key];
             $product->save();
            } // end foreach
